...
function buildAssistPrompt(
  ...
  const defaultSystemPrompt = [
    ...
    "- Ground ALL output in the user's idea brief - never invent generic startup filler.",
    "- Prefer practical constraints, explicit requirements, and measurable success signals.",
    "- Tasks should be concrete implementation steps, not vague goals.",
    "- UX Pack must specify: primary surface, 3-5 key screens, failure states, and responsive design approach.",
    "- If the product has no GUI, explicitly state 'API-only' or 'CLI-only' in UX Pack.",
    "- Keep each field focused and actionable - avoid fluff and marketing language.",
    "",
    "QUALITY CHECKLIST:",
