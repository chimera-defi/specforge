...
function buildAgentHandoff(document: DocumentRecord): string {
  ...
  return [
    ...
    '   - Prompt: "Define user flows with trust moments, failure paths, and conversion points."',
    "2. Architecture Agent",
    "   - Scope: `SPEC.md`, `RISK_REGISTER.md`",
    '   - Prompt: "Define components, data flows, and integration contracts with deterministic replay."',
    "3. Risk Agent",
    "   - Scope: `RISK_REGISTER.md`, `ACCEPTANCE_TEST_MATRIX.md`",
    '   - Prompt: "Enumerate abuse/failure scenarios; specify controls and response playbooks."',
    "4. Validation Agent",
    "   - Scope: `VALIDATION_PLAN.md`, `ACCEPTANCE_TEST_MATRIX.md`",
    '   - Prompt: "Map success signals to measurement methods, source of truth, and thresholds."',
    "",
    "### Merge Contract",
  ...
  const metaChecks: Array<{ key: string; section: string; question: string; decision: string }> = [
    {
      ...
      decision: "Define the core pain point.",
    },
    {
      key: "goals",
      section: "Goals",
      question: "What are the measurable success outcomes?",
      decision: "Each goal must be verifiable.",
    },
    {
      key: "users",
      section: "Users",
      question: "Who are the primary users?",
      decision: "Describe technical level and job.",
    },
    {
      key: "scope",
      section: "Scope",
      question: "What is in scope for the first version?",
      decision: "Define what can ship in 4–6 weeks.",
    },
    {
      key: "requirements",
      section: "Requirements",
      question: "What must the system do? List 3–5 features.",
      decision: "Features needed for first user value.",
    },
    {
      key: "constraints",
      section: "Constraints",
...
function buildRiskRegister(document: DocumentRecord): string {
  ...
  const standardRisks = [
    ...
    },
    {
      ...
      likelihood: "High",
      impact: "Medium",
      mitigation: "Review non-goals at every sprint boundary.",
    },
    {
...
function buildCompetitiveAnalysis(document: DocumentRecord): string {
  ...
  return [
    ...
    "- **Switching Costs**: [Cost to switch]",
    ...
    "- **Buyer Power**: [Buyer influence]",
    "- **Rivalry**: [Competition intensity]",
    "",
    "## Competitive Strategy",
    "- **Offensive**: [Gaining market share]",
    "- **Defensive**: [Protecting position]",
...
function buildBusinessModel(document: DocumentRecord): string {
  ...
  return [
    ...
    "",
    "### Value Propositions",
    "- **Core Value**: [Core value delivered]",
    "- **Pain Points**: [Pain points solved]",
    "- **Gains**: [Gains created]",
    "",
    "### Customer Segments",
    ...
    "- **Secondary**: [Secondary channels]",
    "- **Partners**: [Channel partnerships]",
    "",
    ...
    "- **Intellectual**: [IP, patents, proprietary tech]",
    "- **Human**: [Team roles]",
    ...
    "- **Scale Economies**: [Scale opportunities]",
    "- **Scope Economies**: [Scope opportunities]",
    "",
    "## Pricing Strategy",
    "- **Philosophy**: [Value/cost-plus/competitive]",
    "- **Tiers**: [Pricing tiers]",
    "- **Discounts**: [Discount policies]",
    "- **Upsell/Cross-sell**: [Upsell opportunities]",
    "",
    ...
    "- **LTV:CAC**: [Target ratio, typically 3:1+]",
    "- **MRR**: [Monthly recurring revenue per customer]",
    "- **ARR**: [Annual recurring revenue per customer]",
    "- **Churn Rate**: [Expected churn rate]",
    "- **Gross Margin**: [Expected gross margin]",
    "",
    "## Go-to-Market Strategy",
    "- **Initial Target**: [First segment to target]",
    "- **Marketing Channels**: [Primary marketing channels]",
    "- **Sales Process**: [Sales process overview]",
    "- **Key Partnerships**: [Strategic GTM partnerships]",
    "- **Launch**: [Launch strategy]",
    "- **Growth**: [Post-launch growth strategy]",
    "",
    "## Revenue Milestones",
    "- **Month 1-3**: [Q1 revenue targets]",
    "- **Month 4-6**: [Q2 revenue targets]",
    "- **Month 7-12**: [Year 1 revenue targets]",
    "- **Year 2-3**: [Years 2-3 revenue targets]",
    "",
    "## Business Risks",
    "- **Market**: [Market-related risks]",
    "- **Product**: [Product-related risks]",
    "- **Financial**: [Financial-related risks]",
    "- **Operational**: [Operational-related risks]",
    "- **Regulatory**: [Regulatory-related risks]",
  ].join("\n");
}
